...
def convert_surface_marking_set(input_file, output_file):
    with open(input_file, "r", encoding="utf-8") as f:
        surface_marking_data = json.load(f)

    for marking in surface_marking_data:
        # Convert 'points'
        marking['points'] = [
            utm_to_wgs_custom(p[0], p[1]) + [p[2]]  # Keep Z-coordinate intact
            for p in marking['points']
        ]

    # Save the converted data
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(surface_marking_data, f, indent=4)

    print(f'✅ {input_file} 변환 완료! -> {output_file}')


# 사용 예제
convert_surface_marking_set("surface_marking_set.json", "surface_marking_set_wgs_custom.json")
